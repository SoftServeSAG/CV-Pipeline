...
class FPNInception(nn.Module):
    def __init__(
        ...
        self.fpn = FPN(
            norm_layer=norm_layer, num_filters=num_filters_fpn, pretrained=pretrained
        )
    ...
    def forward(self, x):
        image_logits, map0, map1, map2, map3, map4 = self.fpn(x)
...
class FPN(nn.Module):
    def __init__(self, norm_layer, num_filters=256, pretrained=True):
        ...
        self.inception = inceptionresnetv2(num_classes=1000, pretrained=pretrain)
        ...
        self.enc4 = nn.Sequential(
            self.inception.repeat_1, self.inception.mixed_7a
        )  # 2080
        ...
        self.squeeze = nn.Sequential(
            nn.Conv2d(2080, num_filters * 2, kernel_size=1, stride=1),
            nn.AdaptiveAvgPool2d(1),
        )

        self.image_logits = nn.Sequential(
    ...
    def forward(self, x):
        ...
        squeezed = self.squeeze(enc4)
        image_logits = self.image_logits(squeezed.view(squeezed.size(0), -1))
        ...
        return image_logits, F.pad(lateral0, pad1, "reflect"), map1, map2, map3, map4
